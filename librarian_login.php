...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($conn->connect_error){
            ...
            $sql = "SELECT l_id, l_name, l_password FROM librarian Where l_id ='".$id."'";
            $result = $conn->query($sql);
            $row = mysqli_fetch_assoc($result);
            if (strcasecmp($row["l_id"], $id) == 0) {
               
                if (strcasecmp($row["l_password"], $password) == 0){
                    session_start();
                    $_SESSION['login_user'] = $row["l_name"];
					$_SESSION['login_mode'] = $mode;
					$_SESSION['login_id'] = $row["l_id"];
	...
	<table>
		<tr>
			<th id="box">
			  	<div>
			  		<h4 id="header">LIBRARIAN LOGIN</h4>
			  		<form method="post"   name="librarian_login" autocomplete="off">
						<input type="text" class="inputs" placeholder= "ID"pattern=".{5,}" title="Five or more characters" name="id" required><br>
