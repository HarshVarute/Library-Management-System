...
<?php 
     ...
     if ($_SERVER["REQUEST_METHOD"] == "POST") {
		  ...
		  $dept_id = test_input($_POST['id']);
          $deptname = test_input($_POST['dept_name']);
          if($conn->connect_error){
              die('Connection failed');
          }
          else {
              $sql = "SELECT dept_id FROM department WHERE dept_name = '".$deptname."'";
              $result = $conn->query($sql);
              if($result->num_rows == 0){
				//$last_id = $conn->insert_id+1;
                  $conn -> query("INSERT INTO department(dept_id, dept_name) VALUES ('$_POST[id]', '$_POST[dept_name]')");
                  

                  echo '<script>alert("Department added successfully.\nDepartment Id is '.$dept_id.'")</script>';
                  $conn -> close();
              }
              else{
			...
			<th id="box">
			  	<div>
			  		...
			  		<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>"  name="add_department" autocomplete="off">
						<input type="number" placeholder="dept ID" class="inputs" name="id" required><br><br>
