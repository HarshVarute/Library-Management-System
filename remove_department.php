...
<?php 
     ...
     if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          if($conn->connect_error){
              ...
              $remove = "SELECT dept_id, dept_name FROM department WHERE dept_id = '".$deptid."'";
              $constraint = "SELECT dept_id FROm student WHERE dept_id = '".$deptid."'";
              $const = $conn -> query($constraint);
              ...
              if($result){
                    if($result->num_rows == 0){
                        ...
                        if($const->num_rows == 0){
                            ...
                            $conn -> query("DELETE FROM department WHERE dept_id = '".$deptid."'");
                            echo '<script>alert("Department removed successfully.\nDepartment Name: '.$row["dept_name"].'")</script>';
                            $conn -> close();
                        }
                        else{
                            echo '<script language="javascript">';
                            echo 'alert("You can not delete this Department\nForeign Key constraint!")';
                            echo '</script>';
                        }
                    }
                }
 ...
 ?>
	<table>
		<tr>
			<th id="box">
			  	<div>
			  		<h4 id="header">Remove Department</h4>
			  		<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>"  name="add_department" autocomplete="off">
						...
						<button class="login_button" type="submit">Remove Department</button>
			  		</form>
