                ...
                if($conn->connect_error){
                    ...
                    if($result1->num_rows == 0){
                        ...
                        if($result->num_rows == 0){
                            $conn -> query("INSERT INTO student(std_id, std_name, std_address, std_email, std_contact, dept_id, std_password) VALUES ('$_POST[id]', '$_POST[full_name]', '$_POST[address]', '$_POST[email]', '$_POST[contact]', '$_POST[dept_id]', '$_POST[passwd]')");
                            //$last_id = $conn->insert_id;
                            echo '<script>alert("Student added successfully.\nYour id is '.$std_id.'\nPassword is '.$password.'.")</script>';
                            $conn -> close();
                        }
                        else{
                            echo '<script language="javascript">';
			...
			<th id="box">
			  	<div>
			  		...
			  		<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>"  name="add_lirbrarian" autocomplete="off">
						<input type="number" placeholder="student id" class="inputs" name="id" required><br><br>
						...
						<input type="text" placeholder="Password" class="inputs" name="passwd" required><br><br>
