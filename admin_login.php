...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($conn->connect_error){
            ...
            if (strcasecmp($row["username"], $username) == 0) {
                ...
                $resultp = $conn->query($pass);
                $rowp = mysqli_fetch_assoc($resultp);
                if (strcasecmp($rowp["password"], $password) == 0){
                    session_start();
                    $_SESSION['login_user'] = $username;
					$_SESSION['login_mode'] = $mode;
					$_SESSION['login_id'] = $string;
                    ...
                    echo '<script language="javascript">';
                    echo 'alert("Wrong Username Entered")';
                    echo '</script>';
            }
	...
	<table>
		<tr>
			<th id="box">
			  	<div>
			  		<h4 id="header">ADMIN LOGIN</h4>
			  		<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST"   name="admin_login" autocomplete="off">
						<input type="text" class="inputs" placeholder= "Username"pattern=".{5,}"  name="username" required><br>
						<input type="password" placeholder="Password" class="inputs" pattern=".{5,}"  name="password" required><br><br>
